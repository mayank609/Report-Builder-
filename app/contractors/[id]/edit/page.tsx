...
export default function EditContractorPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: contractor, loading, error, refetch } = useAsync(() => contractorService.getById(id), [id]);
  const [saving, setSaving] = useState(false);
  const [form, setForm] = useState<Partial<Contractor>>({});
  const [initialized, setInitialized] = useState(false);

  if (contractor && !initialized) { setForm(contractor); setInitialized(true); }

  const update = (patch: Partial<Contractor>) => setForm((f) => ({ ...f, ...patch }));
  ...
  if (error || !contractor) return <ErrorState title="Contractor not found" onRetry={refetch} />;
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      <div>
        ...
        <h1 className="text-xl font-semibold text-foreground">Edit Contractor</h1>
      </div>

      <Card className="py-5">
        <CardHeader className="px-5">
          <CardTitle className="text-base">Contractor Details</CardTitle>
          <CardDescription>Update contractor information.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4 px-5">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-1.5">
              ...
              <Input id="trade" value={form.trade || ""} onChange={(e) => update({ trade: e.target.value })} />
            </div>
            <div className="space-y-1.5">
